defmodule Postern.PgIdentDiagnostics do
  ...
  def diagnostics(text, hba_text \\ nil) when is_binary(text) do
    ...
    referenced = referenced_maps(hba_text)
  ...
  defp referenced_maps(nil), do: MapSet.new()

  defp referenced_maps(text) do
    {:ok, entries} = PgHba.parse(text)

    entries
    |> Enum.filter(&(&1.type == :rule and &1.auth_method == "ident"))
